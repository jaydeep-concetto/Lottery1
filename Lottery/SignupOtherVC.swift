...
class SignupOtherVC: BaseViewController {
    ...
    @IBAction func btnSignupEmailClicked(_ sender: UIButton) {
        ...
        {
            postapi(url: URL_NAME.register, maindict:["email":txtEmail.text!,"password":txtPassword.text!]) { (dict) in
                if dict.count != 0
                {
